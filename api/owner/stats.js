...
export default async function handler(req, res) {
  ...
  try {
    ...
    res.json({
      users: { total: profiles?.length||0, pro: profiles?.filter(p=>p.plan==='pro').length||0, trial: profiles?.filter(p=>p.plan==='trial').length||0, free: profiles?.filter(p=>p.plan==='free').length||0 },
      revenue: { total_inr: totalRevenue/100, this_month_inr: monthRevenue/100, total_payments: payments?.length||0 },
